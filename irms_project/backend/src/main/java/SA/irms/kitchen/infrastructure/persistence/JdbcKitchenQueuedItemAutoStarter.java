...
@Component
@Order(20)
public class JdbcKitchenQueuedItemAutoStarter implements KitchenAutomationStep {
    ...
    JdbcKitchenQueuedItemAutoStarter(JdbcClient jdbcClient, KitchenTicketItemRepositoryPort ticketItemRepository,
            KitchenTicketItemWorkflowPort itemWorkflowService, KitchenTicketStateCoordinatorPort ticketStateCoordinator,
    ...
    public void execute() {
        ...
        List<UUID> itemIds = jdbcClient.sql("""
                        ...
                        where status = 'queued' and next_action_at is not null and next_action_at <= :cutoff
                        """)
        ...
        itemIds.forEach(ticketItemId -> {
            ...
            if (!"queued".equals(itemContext.status())) {
                return;
            }
            itemWorkflowService.moveItemToCooking(itemContext, actorUserId, "system:auto-cook");
