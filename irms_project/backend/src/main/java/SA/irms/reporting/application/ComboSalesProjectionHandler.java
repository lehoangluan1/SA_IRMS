...
@Component
public class ComboSalesProjectionHandler implements ReportingProjectionHandler {
    private final ReportingProjectionRepository repository;

    public ComboSalesProjectionHandler(ReportingProjectionRepository repository) {
    ...
    public boolean supports(EventEnvelope envelope) {
        return ServiceEventTypes.ORDER_CONFIRMED.equals(envelope.metadata().eventType())
                && envelope.payload().containsKey("combos");
    }

    @Override
    public void project(EventEnvelope envelope) {
        repository.materializeComboSalesProjection(envelope);
    }
}
